...
def save_function(headline_list):
    ...
    """
    source = headline_list[0]['source']
    new_count = 0

    error = True
    try:
        latest_headline = Headline.objects.filter(source=source).order_by('-id')[0]
